...
select_artist_folder() {
    ...
    local folders=()
    [ -d "$BASE_DIR/$DEFAULT_ARTIST_DIR" ] && folders+=("$DEFAULT_ARTIST_DIR")
...
input_album_artist() {
    ...
    else echo "$input"; echo "✅ Album Artist: $input (自定义)" >&2; fi
}
...
extract_song_info() {
    ...
    if [[ "$title" =~ ^(.+)\ -\ (.+)$ ]]; then
        ...
        song_name=$(echo "$song_name" | sed 's/\s*\[[^]]*\]//g' | sed 's/\s*(Official[^)]*)//g' | sed 's/\s*(Acoustic[^)]*)//g' | sed 's/\s*(Lyric[^)]*)//g' | sed 's/\s*(Live[^)]*)//g' | sed 's/\s*(Performance[^)]*)//g' | sed 's/\s*(Stripped[^)]*)//g' | xargs)
        echo "${song_name}|${song_artist}"; return
    fi
    if [[ "$title" =~ 《([^》]+)》 ]]; then echo "${BASH_REMATCH[1]}|"; return; fi
    if [[ "$title" =~ \"([^\"]+)\" ]]; then echo "${BASH_REMATCH[1]}|"; return; fi
    local cleaned=$(echo "$title" | sed 's/^【[^】]*】//' | sed 's/^Stage: //' | sed 's/^纯享[：:]//' | xargs)
    echo "${cleaned:0:50}|"
}
...
for idx in "${!VALID_URLS[@]}"; do
    ...
    ALBUM_CONFIGS+=("$SAFE_NAME|$SELECTION|$url|$FINAL_PATH|$ALBUM_ARTIST|$ENHANCED_MODE|$TYPE|$HAS_METADATA|$MV_TITLE|$MV_ARTIST|$MV_ALBUM|$MV_ALBUM_ARTIST|$BATCH_ALBUM|||$NORMAL_SELECTION|$MV_VIDS|$MV_INFO|$MV_STRATEGY")
    echo ""
done
...
SLEEP_REQUESTS="$PLAYLIST_SLEEP_REQUESTS"
SLEEP_INTERVAL="$PLAYLIST_SLEEP_INTERVAL"
...
for album_entry in "${ALBUMS[@]}"; do
    ...
    if [ "$MV_STRATEGY" = "2" ]; then
        ...
        for f in "$FINAL_PATH"/*.opus; do
            ...
            TITLE=""
            [ -f "$JSON_FILE" ] && TITLE=$(python3 -c "import json; print(json.load(open('$JSON_FILE')).get('title',''))" 2>/dev/null)
            CF=""
            if [ -f "$JSON_FILE" ]; then
                if download_cover "$JSON_FILE" CF; then
                    CC="/tmp/cover_$$_compressed.jpg"
    ...
    if [ -n "$MV_VIDS" ] && [ "$MV_STRATEGY" = "1" ]; then
        ...
        for mv_f in "$FINAL_PATH"/temp_mv_*.opus; do
            ...
            VID=""
            [ -f "$JSON_FILE" ] && VID=$(python3 -c "import json; print(json.load(open('$JSON_FILE')).get('id',''))" 2>/dev/null)
    ...
    for f in "$FINAL_PATH"/*.opus; do
        ...
        if [ "$ENHANCED_MODE" = "true" ]; then
            JSON_FILE="${f%.opus}.info.json"
            if [ -f "$JSON_FILE" ]; then
                ORIG_ALBUM=$(python3 -c "import json; print(json.load(open('$JSON_FILE')).get('album',''))" 2>/dev/null)
                SA=$(python3 -c "import json; print(json.load(open('$JSON_FILE')).get('artist',''))" 2>/dev/null)
                HS=false
